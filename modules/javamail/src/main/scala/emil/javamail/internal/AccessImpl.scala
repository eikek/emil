...
final class AccessImpl[F[_]: Sync: ContextShift](blocker: Blocker) extends Access[F, JavaMailConnection] {
  def getInbox: MailOp[F, JavaMailConnection, MailFolder] =
    FindFolder(None, "INBOX").blockOn(blocker).andThen(optMf => optMf match {
      case Some(mf) => mf.pure[F]
      case None => Sync[F].raiseError(new Exception("Folder INBOX doesn't exist."))
    })

  def createFolder(parent: Option[MailFolder], name: String): MailOp[F, JavaMailConnection, MailFolder] =
    CreateFolder(parent, name).blockOn(blocker)

  def findFolder(parent: Option[MailFolder], name: String): MailOp[F, JavaMailConnection, Option[MailFolder]] =
    FindFolder[F](parent, name).blockOn(blocker)

  def getMessageCount(folder: MailFolder): MailOp[F, JavaMailConnection, Int] =
    MailOp(conn => conn.folder[F](folder.id).use { f =>
      Sync[F].delay(f.getMessageCount)
    })

  def search(folder: MailFolder, max: Int)(query: SearchQuery): MailOp[F, JavaMailConnection, SearchResult[MailHeader]] =
    SearchMails(folder, query, max).blockOn(blocker)


  def searchAndLoad(folder: MailFolder, max: Int)(query: SearchQuery): emil.MailOp[F, JavaMailConnection, SearchResult[Mail[F]]] =
    SearchMails.load(folder, query, max)

  override def searchDelete(folder: MailFolder, max: Int)(query: SearchQuery)(implicit ev: FlatMap[F]): MailOp[F, JavaMailConnection, DeleteResult] =
